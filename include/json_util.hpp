...
template <Numeric T> std::string ToJSON(const T &x);
template <typename T, typename U>
inline std::string ToJSON(const std::map<T, U> &m);
template <typename T> std::string ToJSON(const std::vector<T> &m);

inline std::string ToJSON(const media_observer::ObservingFileType &x) {
  return "\"" +
         std::string(media_observer::OBSERVING_TYPES_NAMES.at(
             media_observer::FileTypeToId(x))) +
         "\"";
}

inline std::string ToJSON(const std::string &x) { return '"' + x + '"'; }
