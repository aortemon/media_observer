...
MediaObserver::MediaObserver(const std::chrono::seconds& interval) : interval_(interval) {
    GetHomeDirectory();
}

MediaObserver::MediaObserver(const std::chrono::seconds& interval, std::filesystem::path directory)
...
void MediaObserver::Update() {
    if (!std::filesystem::exists(directory_) || !std::filesystem::is_directory(directory_)) {
        throw std::runtime_error(directory_.string() + " doesnot exist or is not a directory");
    }
    files_.clear();
...
std::map<ObservingFileType, std::vector<std::string>> MediaObserver::GetState() const {
    return files_;
};
