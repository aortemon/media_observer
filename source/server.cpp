...
std::string Server::MakeErrorResponse(int status_code, const std::string& message) {
    std::string response;
    if (status_code == 405) {
...
bool Server::SetUpSocket() {
    ...
    int opt = 0;
    if (setsockopt(server_, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt)) != 0) {
    ...
    if (bind(server_, reinterpret_cast<struct sockaddr*>(&address_), sizeof(address_)) < 0) {
        std::cerr << "[ERROR] Failed to bind socket to port. Check if it's not already in use\n";
        return false;
    }
...
Server::Server(int port)
    : port_(port), is_running_(false), server_(-1), addrlen_(sizeof(address_)), observer_(nullptr) {
...
bool Server::Serve() {
    ...
    is_running_ = true;
    std::cout << "\n\nServer listening on: http://localhost:" << port_ << "/media_files\n";
